...
for (i in precinct_no){
	...
	precinct_date_count$date <- as.yearmon(precinct_date_count$date)
	...
	precinct_date_count_xts <- xts(precinct_date_count$count, precinct_date_count$date)

	precinct_month_count[[i]] <- apply.monthly(precinct_date_count_xts,sum)
	#precinct_week_count[[i]] <- apply.weekly(precinct_date_count_xts[[i]], sum)
	
	#Insert zeros into gaps in the data
	precinct_month_df <- data.frame(index(precinct_month_count[[i]]),precinct_month_count[[i]])
	precinct_month_merge <- merge(full,precinct_month_df,by.x=1,by.y=1,all=T)[-2]
	precinct_month_merge[is.na(precinct_month_merge[2]==T),2] <- 0

	#Convert data to time series
	precinct_month_count_xts <- xts(precinct_month_merge[2],precinct_month_merge[,1])
	...
	#precinct_week_count_ts[[i]] <- ts(as.numeric(precinct_week_count[[i]]),as.numeric(format(start(precinct_week_count[[i]]),"%Y")),frequency = 52)
	...
	#precinct_decomposed_log_week[[i]] <- stl(log(precinct_week_count_ts[[i]]),s.window=5,s.degree=0,t.window=19,robust=TRUE)
